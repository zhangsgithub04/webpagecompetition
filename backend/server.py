...
app = FastAPI(title="Webpage Competition API", version="1.2.0")
...
def verify_file_exists(bucket_name: str, folder: str, expected_filename: str) -> None:
    try:
        files = supabase.storage.from_(bucket_name).list(folder)
        found = False
        if isinstance(files, list):
            for item in files:
                if isinstance(item, dict) and item.get("name") == expected_filename:
                    found = True
                    break
        if not found:
            raise HTTPException(
                status_code=500,
                detail=f"Upload verification failed for {bucket_name}/{folder}/{expected_filename}",
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Storage verification failed for bucket '{bucket_name}': {str(e)}",
        )
...
def upload_to_bucket(bucket_name: str, uploaded_file: UploadFile, folder: str) -> str:
    try:
        ...
        verify_file_exists(bucket_name, folder, filename)
        return file_path
...
def detect_image_media_type(path: str) -> str:
    lower = path.lower()
...
@app.get("/files/avatar-image", dependencies=[Depends(verify_api_key)])
def get_avatar_image(path: str = Query(...)) -> Response:
    try:
        content = supabase.storage.from_(AVATAR_BUCKET).download(path)
        if not content:
        ...
        raise HTTPException(status_code=500, detail=f"Avatar image failed: {str(e)}")


@app.get("/files/spa-html", dependencies=[Depends(verify_api_key)])
def get_spa_html(path: str = Query(...)) -> Response:
    try:
        content = supabase.storage.from_(SPA_BUCKET).download(path)
        if not content:
        ...
        raise HTTPException(status_code=500, detail=f"SPA HTML failed: {str(e)}")
...
def update_entry_upload(
    ...
    try:
        ...
        current_entry = ensure_single(existing)
        ...
        update_data["avatar_file_path"] = current_entry.get("avatar_file_path")
        update_data["spa_file_path"] = current_entry.get("spa_file_path")
