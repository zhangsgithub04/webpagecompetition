...
app = FastAPI(title="Webpage Competition API", version="1.1.0")
...
def extract_signed_url(result: Any) -> Optional[str]:
...
def get_avatar_url(path: str = Query(...)) -> Dict[str, str]:
    try:
        ...
        signed_url = extract_signed_url(result)

        if not signed_url:
            raise HTTPException(status_code=400, detail=f"Could not create avatar URL: {result}")
...
def get_spa_url(path: str = Query(...)) -> Dict[str, str]:
    try:
        ...
        if not signed_url:
            raise HTTPException(status_code=400, detail=f"Could not create SPA URL: {result}")
